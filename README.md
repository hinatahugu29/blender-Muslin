...
| 頂点数 | 制約のみ | + コライダー | + 自己衝突 |
|--------|---------|------------|-----------|
| 1,681 | 1.9 ms | 4.6 ms | 5.5 ms |
| 4,225 | 4.7 ms | 7.6 ms | 9.0 ms |
| 8,281 | 9.5 ms | 14.9 ms | 16.9 ms |
| 14,641 | 17.1 ms | 26.0 ms | 29.5 ms |

14,641頂点で全部入りでも 34 fps 相当です。内訳:
...
**いまの最大項目は曲げ制約**です。二面角を扱う都合で1制約あたり4頂点に触るため、
以前の距離制約による近似(剛性の制御として機能していなかった)より 4倍ほど高価に
なっています。次に手を入れるならここです。

自己衝突はもう支配的ではありません。空間ハッシュを HashMap から計数ソート方式
(再利用できる平坦な配列3本)に変えて、再構築が 4.2ms から 0.7ms になりました。
