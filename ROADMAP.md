...
- 2026-09-14 の実機確認で分かったこと(実測は README「パラメータの決め方」に記載):
  ...
  - 衝突後に伸び制約を解き直す `post_collision_iterations` を追加(既定 2)。
    貫通を増やさず伸び誤差を減らす。
- **要検討(未着手)**: `resolve_self_collisions` が毎サブステップで SpatialHash を
  作り直している(`SpatialHash::new` + `rebuild`)。近傍リストも頂点ごとに Vec を確保。
  自己衝突のコストが約3倍になる主因の可能性があり、**アロケーション再利用だけで
  CPU のまま改善するなら M4(GPU化)の優先度が下がる**。先に計測すること。
