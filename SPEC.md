...
| 並列化 | **rayon** | 伸び・曲げ制約のブロック色分け並列求解、衝突判定（4,000 頂点以上） |
...
M9 で伸び・曲げの制約をブロックごとに色分けして並列に解くようにし、14,641 頂点の
制約のみが 37.5ms → 8.0ms になった（同じ環境で測り直した値）。**いまの最大項目は
自己衝突**（とくに三角形の空間ハッシュの再構築）。
...
**ただし前提が2度変わっている。** 計画当時は「自己衝突がボトルネック」、その後の
最適化で「制約ソルバ本体がボトルネック」になり、M9（CPU でのブロック色分け並列）で
制約は 4〜5 倍速くなった。約3万頂点・自己衝突ありでも 43ms（Normal 相当・球に
被せた場面で 146ms）なので、**GPU 化は実測で必要と分かってから**にする
（ROADMAP「M7 以降の方向」）。
