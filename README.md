...
自己衝突のコスト(球コライダーあり、繰り返し5回の最小値):

| 頂点数 | 自己衝突 | うち空間ハッシュ | うち列挙と押し出し |
|--------|---------|----------------|------------------|
| 4,225 | 2.1 ms | 1.1 ms | 1.0 ms |
| 8,281 | 3.8 ms | 2.4 ms | 1.5 ms |
| 14,641 | 6.6 ms | 4.5 ms | 2.1 ms |

2段階で速くした。もともと 14,641頂点で 41.3ms かかっていた。

1. 空間ハッシュのキー `(i64,i64,i64)` に標準の SipHash を使っていたのを
   軽量ハッシャに差し替え、近傍バッファも使い回すようにした(-64%)
2. 近傍の**列挙**を並列化した(-59%)。列挙は読み取りだけなので並列にでき、
   押し出しは頂点対が互いに書き込むので逐次のまま残している。
   区切り方を固定して結合順を揃えてあるので、結果は決定的なまま

合わせて **41.3ms → 6.6ms(6.3倍)**。いまは空間ハッシュの再構築が
自己衝突コストの 68% を占めており、次に手を入れるならそこ。
